...
export async function readFilesFromDir(dir) {
  ...
  const mapped = files.map(async (file) => {
    ...
    console.log('mapping', file, path);// console.log('mapping', file, path);
    const info = await stat(path);
    console.log('info', info);// console.log('info', info);

    if (info.isDirectory()) {
      console.log('is dir');// console.log('is dir!');
      return null;
    }

    if (info.isFile()) {
      console.log('is file!');// console.log('is file!');
      return path;
    }
  ...
  console.log('resolving promises...')// console.log('resolving promises...');
  const resolved = await Promise.all(mapped);
  console.log('resolved!', resolved)// console.log('resolved!', resolved);
